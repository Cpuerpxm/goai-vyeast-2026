...
DEPRECATED = {
    ...
    # ---- 2026-08-26：**两位小数的四舍五入形式也要登记**。
    #      「总分从 0.44 推到 0.47」在 docs/21 里挂了一整轮：0.44/0.47 是
    #      0.4406/0.4694 的两位小数形式，作废清单只登记了四位小数版，
    #      正向出处核对又因为它「匹配到某个已登记值的两位小数写法」而放行。
    #      两道防线各漏一半，合起来等于没防住。
    "0.4694": ("更正前的 C-free 总分（四位小数）", "现为 0.5008"),
    "0.4406": ("更正前的满秩 ridge 总分（四位小数）", "现为 0.4817"),
    # ---- 2026-08-25：评分器指标 1 改回逐样本之后，违规量化那张表整体重算。
    #      旧值在 §7.3 里挂了一整轮没人发现，因为第三道防线当时根本没对复赛材料跑过。
    "0.502948": ("拟合行泄漏对照表里更正前的旧值（全部 train_val）", "现为 0.529318"),
    "0.469388": ("拟合行泄漏对照表里更正前的旧值（只用 train 折）", "现为 0.500797"),
...
ALLOW = {
    ...
    # ---- 2026-08-25：已提交的初赛稿是**历史存档**，扫它属于范围错误。
    # 那份稿子 2026-08-07 提交，里面的数在当时是对的（LOCO 宇宙含 val 化合物那一版）；
    # 现在拿改正后的清单去核当日的稿，必然命中，而那份稿子按设计与提交定稿逐字节一致、
    # 不允许改动。这条不是为了让检查通过——检查本来就不该管存档。
    # 干净环境复现测试查出：公开仓库里有这份稿子，本机没有，
    # 于是本机一路绿灯，评委照 README 跑却会看到一步 FAIL。
    "初赛方案说明文档.md",
}
...
ALLOW_SECTION_MARKERS = [
    ...
    # 2026-08-25 补："更正前"是"订正前"的同义写法，材料里两种都用过
    "更正前", "旧值", "已作废",
]
TEXT_EXT = {".md", ".txt"}
