...
def main() -> None:
    ...
    def run(phi_mode: str, seed: int = 0, lam_r: float = None) -> dict:
        ...
        for fd in folds:
            ...
            if phi_mode == "none":
                ...
                # 目标派生参考：直接用留出化合物**自己**的平均残差当 Delta。
                # ❗它**不是**任何意义上的上界（2026-08-25 · GPT Pro R6 L1-02）。
                # 算术均值只对**平方误差**最优，不是平均 Pearson 相关的最大化解。
                # 反例（正交基下）：同一化合物两条响应 (10,0) 与 (0,1)，
                # 自身均值模板的平均 PCC = 11/(2*sqrt(101)) ~ 0.547，
                # 而固定模板 (1,1)/sqrt(2) 拿到 1/sqrt(2) ~ 0.707，更高。
                # 所以它连「固定单谱模型在样本轴平均 PCC 下的上界」都不是。
                # 只能报作「用真值构造的一个参考点」，不得称上限/天花板。
                R2 = ctx.X - B
                y = B.copy()
                for c in np.unique(pert[te]):
                    sel = te & (pert == c)
    ...
    slf = run("oracle_self")
    a(f"  {'[参考] 神谕用自身平均残差':<28}{slf['abs_pcc']:>10.4f}"
    ...
    a(f"  【目标派生的自身均值模板参考】用留出化合物自己的平均残差：{gain_s:+.4f}")
    a(f"    按指标 2 的 25% 权重折算约 {0.25 * gain_s:+.4f}。")
    a("    ⚠ 这**不是**上界。算术均值只对平方误差最优，不是平均 Pearson 相关的最大化解；")
    a("      正交基下两条响应 (10,0) 与 (0,1)：自身均值模板平均 PCC ~ 0.547，")
    a("      而固定模板 (1,1)/sqrt(2) 拿到 ~ 0.707。它连固定单谱模型的上界都不是。")
    a("    ⚠ 折算值假设其余五项不受影响，未经检验，既不是总分估计也不是任何上界。")
    a("    正确读法：这只是一个用留出真值构造出来的参考点，用于给可实现方案一个比照。")
    a("")
    ...
    if gain_f < 0.005:
        ...
        a("    换成有信息的特征就拿得到增益 —— **模块没有被正则压成 0**。")
        a("")
        a("    ⚠ 它能排除的只有这一条（GPT Pro R6 · L2-02 收窄）。")
        a("      神谕特征是目标派生、与响应对齐、低维且高信噪比的表示，")
        a("      在维数、几何、正则作用与样本效率上都与 ECFP 不同。")
        a("      所以不能由它推出「ECFP 的失败确实就是表示内容的问题」——")
        a("      能说的是：**「模块完全失活」这一狭义解释已被排除**。")
    a("")
    if gain_o < 0.005:
