...
| `audit/train_boundary_probe.py` | `899489c5441289af` |
| `data/control_match.py` | `bf63ab055a32f7fd` |
| `data/desensitize.py` | `30fc801e30ffdcfb` |
| `data/doc_number_check.py` | `610fc4e408a4b7a6` |
...
| `models/loco_response.py` | `2545021b066df948` |
...
|---|---|---|---|---|
| B0 全局均值谱 | 0.3770 | 0.9535 | 0.8831 | 0.1872 |
| B1 预测=匹配对照 | 0.3122 | 0.9878 | 0.9745 | 0.0000 |
| B2g 总体平均响应 | 0.3373 | 0.9879 | 0.9747 | 0.0502 |
| B2 上下文均值响应 | 0.3393 | 0.9881 | 0.9750 | 0.1720 |
| B4 ridge 响应 | 0.3682 | 0.9881 | 0.9750 | 0.1697 |
| B3 化学近邻响应 | 0.3208 | 0.9870 | 0.9727 | 0.0311 |
| B3o 神谕近邻(目标派生参考) | 0.3518 | 0.9875 | 0.9737 | 0.1150 |
| 对照收缩 α=0.15 | 0.4126 | — | — | — |
...
|---|---|---|---|---|---|---|
| 全局均值谱 | 0.3770 | 0.9535 | 0.8831 | 0.1872 | 0.1324 | 0.2252 |
| 逐蛋白 ridge(满秩) | 0.4817 | 0.9819 | 0.9626 | 0.3052 | 0.2718 | 0.3417 |
| 低秩 K0=16 + ridge | 0.5008 | 0.9844 | 0.9675 | 0.3295 | 0.3115 | 0.3509 |

### 端到端选秩曲线

| K0 | total | abs_r2 | fc_pcc |
|---|---|---|---|
| K0=8 | 0.4856 | 0.9638 | 0.3108 |
| K0=16 | 0.5008 | 0.9675 | 0.3295 |
| K0=32 | 0.5005 | 0.9673 | 0.3290 |
| K0=96 | 0.4992 | 0.9669 | 0.3277 |

### 配对 bootstrap：低秩 K0=16 − 满秩 ridge

- 差值均值 **+0.0185**
- 95% 区间 **[+0.0103, +0.0222]**，排除 0
- 500 次重抽样，按 46 个化合物分组，低秩更优的比例 100.0%
...
|---|---|---|---|
| 仅上下文 b | 0.3739 | 0.3505 | 0.3115 |
| + ECFP | 0.3740 | 0.3509 | 0.3118 |
...
- 实测：drug_resid -0.0010 · 总分 -0.0007 · abs_r2 -0.0004 · S1 abs_r2 +0.0000
- **裁决：不保留，按预案永久停止该路线**
